...
import {
  ...
  Copy, Check, Download, Share2, Languages
} from "lucide-react";
...
function ResultViewer({ rows, apiName, query, cached }: {
  rows: ResultRow[];
  apiName: string;
  query: string;
  cached: boolean;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="px-3 pb-4 space-y-1.5 max-h-[50vh] overflow-y-auto">
        {display.length === 0 ? (
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-1 min-w-0 items-center justify-between gap-2">
                ...
                <button
                  onClick={() => handleCopy(row.key, row.value)}
                  className="hidden sm:block flex-shrink-0 text-muted-foreground hover:text-primary"
                >
                  {isCopied
                    ? <Check className="w-3.5 h-3.5 text-green-400" />
                    : <Copy className="w-3.5 h-3.5" />
...
export default function Home() {
  ...
  const [result, setResult] = useState<{ data: Record<string, unknown>; cached: boolean; apiName: string; success: boolean; error?: string } | null>(null);
  ...
  const handleLookup = () => {
    ...
    performLookup.mutate({ data: { slug: selectedApiSlug, query } }, {
      ...
      onError: (error: unknown) => {
        ...
        toast({
          title: "Lookup Failed",
          description: e?.data?.error || "An unknown error occurred",
          variant: "destructive"
        });
      }
    });
  ...
  const rows = result ? flattenData((result.data as Record<string, unknown>) || { error: result.error || "Unknown error" }) : [];
    ...
    <Layout>
      <div className="max-w-5xl mx-auto space-y-6">
        <header className="mb-6">
          ...
          <p className="text-muted-foreground mt-2 text-sm">Initialize target acquisition protocol.</p>
        </header>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {CATEGORIES.map(cat => (
            <Badge
              ...
              className="cursor-pointer hover:bg-primary/20 transition-colors px-3 py-1 text-xs"
        ...
        <div className="grid md:grid-cols-3 gap-4 md:gap-6">
          <div className="md:col-span-1 order-2 md:order-1 space-y-3 max-h-[50vh] md:max-h-[600px] overflow-y-auto pr-1">
            {isLoadingApis ? (
              <div className="space-y-3">
                {[1,2,3,4,5].map(i => (
              ...
              filteredApis.map(api => (
                ...
                >
                  ...
                  <div className="text-xs text-primary/70 mt-1">{api.command}</div>
          ...
          <div className="md:col-span-2 order-1 md:order-2 space-y-4">
            <Card className="border-border shadow-lg bg-card/80 backdrop-blur">
              <CardHeader className="border-b border-border bg-black/20 pb-4 px-4 pt-4">
                <CardTitle className="text-base sm:text-lg flex items-center gap-2">
                  <Cpu className="w-5 h-5 text-primary flex-shrink-0" />
                  <span className="truncate">{selectedApi ? `Target: ${selectedApi.name}` : "Select a target vector"}</span>
                </CardTitle>
                {selectedApi && (
                ...
                <div className="flex gap-2 sm:gap-3">
                  <div className="relative flex-1">
                    ...
                    <Input
                      ...
                      placeholder={selectedApi ? `Enter ${selectedApi.example}...` : "Select a vector first..."}
                      className="pl-10 bg-black/50 border-border focus-visible:ring-primary font-mono text-sm"
                      autoComplete="off"
                      spellCheck="false"
                    />
                  </div>
                  <Button
                    ...
                    className="bg-primary text-primary-foreground hover:bg-primary/90 px-4 sm:w-32 font-bold tracking-widest text-sm"
                  >
              ...
              <Card className="border-primary/30 shadow-[0_0_15px_rgba(0,217,255,0.05)] bg-card animate-in fade-in slide-in-from-bottom-4 overflow-hidden">
                <CardHeader className="border-b border-border pb-3 px-4 pt-4">
                  <div className="flex items-center gap-2">
                    ...
                    {result.success && (
                      <span className="ml-auto text-xs text-muted-foreground font-normal flex-shrink-0">{lastQuery}</span>
                    )}
                  </div>
                </CardHeader>
